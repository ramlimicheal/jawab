...
export default function RegisterPage() {
  ...
  return (
    <div className="h-screen max-h-[1200px] w-full bg-white flex flex-col md:flex-row antialiased font-sans overflow-hidden">
      {/* Left Side */}
      ...
      <div className="w-full md:w-[60%] xl:w-[65%] p-6 md:p-10 lg:p-12 flex flex-col justify-center bg-white h-full overflow-y-auto custom-scrollbar">
        <div className="max-w-[400px] xl:max-w-[420px] w-full mx-auto flex flex-col justify-center min-h-0 h-full">
          ...
          <div className="shrink-0 mb-4 xl:mb-6">
            <h1 className="text-2xl xl:text-3xl font-extrabold text-gray-900 mb-1 tracking-tight">Create your account</h1>
            <p className="text-[13px] xl:text-[14px] text-gray-500 font-medium leading-relaxed pr-6">
              Start your 14-day free trial. No credit card required.
            </p>
          </div>

          <div className="shrink-0 flex-1 flex flex-col justify-center space-y-4 xl:space-y-5">
            {/* OAuth Buttons */}
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-3 xl:space-y-4 shrink-0">
              {error && (
              ...
              <div className="space-y-1">
                <Label htmlFor="name" className="text-[12px] xl:text-[13px] font-bold text-gray-900">FullName</Label>
                <Input
                  id="name"
                  type="text"
                  placeholder="e.g. Dr. Khalid Al-Mansouri"
                  className={`h-10 xl:h-11 text-[13px] xl:text-[14px] bg-gray-50/50 border-gray-200 focus:border-emerald-500 focus:ring-emerald-500 rounded-xl px-4 ${errors.name ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : ''}`}
                  {...register("name")}
                />
                {errors.name && (
                  <p className="text-[11px] xl:text-[12px] text-red-500 font-medium">{errors.name.message}</p>
                )}
              </div>
              ...
              <div className="flex items-start justify-between py-2 xl:py-3 border-t border-b border-gray-50 my-2">
                <div className="pr-4">
                  <Label htmlFor="updates" className="text-[12px] xl:text-[13px] font-bold text-gray-900 cursor-pointer mb-[2px] block">
          ...
          <div className="mt-4 xl:mt-6 text-center shrink-0">
            <span className="text-[12px] xl:text-[13px] text-gray-500 font-medium">Already have an account? </span>
            <Link href="/auth/login" className="text-[12px] xl:text-[13px] font-bold text-emerald-600 hover:underline">
              Sign in
            </Link>
          </div>

          <div className="mt-6 text-center text-[10px] xl:text-[11px] text-gray-400 font-medium tracking-wide shrink-0">
